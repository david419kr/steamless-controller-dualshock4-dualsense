...
이 브랜치는 Steam Controller를 가상 Xbox 360, DualShock 4, 실험적 DualSense, 실험적 Switch Pro Controller, 또는 실험적 Switch 2 Pro Controller로 노출하는 SteamlessController 포크입니다. 목표는 기존 Xbox 360 동작을 유지하면서, PlayStation/Nintendo 네이티브 입력 경로가 필요한 게임에서 자이로, 터치패드, 진동, 트랙패드 햅틱, DualShock 4 / DualSense / Switch Pro / Switch 2 Pro 지원을 사용할 수 있게 하는 것입니다.
...
4. 트레이 메뉴에서 `Output Mode -> DualShock 4`, `Output Mode -> DualSense`, `Output Mode -> Switch Pro Controller`, 또는 `Output Mode -> Switch 2 Pro Controller`를 선택하고 `Enable Steamless Mode`를 켭니다.
...
- Switch Pro Controller 모드는 실험적 기능이며 버튼, 스틱, ZL/ZR, 네이티브 자이로/가속도, Capture, 해석 기반 HD Rumble best-effort 변환을 지원합니다.
...
패치된 `viiper.exe`를 배포 artifact에 포함하는 경우 VIIPER의 GPLv3 라이선스 고지와 대응 소스/패치 위치를 함께 제공해야 합니다. `tools\build-viiper.ps1`은 출력 폴더에 `VIIPER-LICENSE.txt`, `VIIPER-SOURCE.txt`, DS4/DualSense/Switch Pro/Switch 2 Pro patch 파일과 `nspro-overlay`, `ns2pro-overlay`를 같이 복사합니다.
