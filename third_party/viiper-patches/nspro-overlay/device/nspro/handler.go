package nspro

import (
	"encoding/json"
	"fmt"
	"io"
	"log/slog"
	"net"

	"github.com/Alia5/VIIPER/device"
	"github.com/Alia5/VIIPER/internal/server/api"
	"github.com/Alia5/VIIPER/usb"
)

func init() {
	api.RegisterDevice("nspro", &handler{})
}

type handler struct{}

var serials = map[string]struct{}{}

func (h *handler) CreateDevice(o *device.CreateOptions) (usb.Device, error) {
	if o == nil {
		o = &device.CreateOptions{}
	}

	metaState := *defaultMetaState()
	if o.DeviceSpecific != nil {
		data, err := json.Marshal(o.DeviceSpecific)
		if err != nil {
			return nil, fmt.Errorf("marshal device specific args: %w", err)
		}
		if err := json.Unmarshal(data, &metaState); err != nil {
			return nil, fmt.Errorf("invalid device specific JSON: %w", err)
		}
	}

	serial := metaState.SerialNumber
	if serial == "" {
		serial = DefaultSerial
	}
	if _, ok := serials[serial]; ok {
		if len(serial) < 4 {
			serial = DefaultSerial
		}
		for i := 1; i < 16; i++ {
			newSerial := fmt.Sprintf("%s%04X", serial[:len(serial)-4], i)
			if _, exists := serials[newSerial]; !exists {
				serial = newSerial
				break
			}
		}
	}

	metaState.SerialNumber = serial
	serials[serial] = struct{}{}

	b, err := json.Marshal(metaState)
	if err != nil {
		return nil, fmt.Errorf("marshal meta state: %w", err)
	}
	var deviceSpecific map[string]any
	if err := json.Unmarshal(b, &deviceSpecific); err != nil {
		return nil, fmt.Errorf("unmarshal meta state args: %w", err)
	}
	o.DeviceSpecific = deviceSpecific

	return New(o)
}

func (h *handler) StreamHandler() api.StreamHandlerFunc {
	return func(conn net.Conn, devPtr *usb.Device, logger *slog.Logger) error {
		defer func() {
			if devPtr == nil || *devPtr == nil {
				return
			}
			ns, ok := (*devPtr).(*NSPro)
			if !ok {
				slog.Warn("device is not nspro on disconnect")
				return
			}
			serial := ns.metaSerialNumber()
			if serial == "" {
				return
			}
			delete(serials, serial)
			slog.Debug("nspro disconnected, serial released", "serial", serial)
		}()

		if devPtr == nil || *devPtr == nil {
			return fmt.Errorf("nil device")
		}
		ns, ok := (*devPtr).(*NSPro)
		if !ok {
			return fmt.Errorf("device is not nspro")
		}

		clearOutputCallback := ns.SetOutputCallback(func(feedback OutputState) {
			data, err := feedback.MarshalBinary()
			if err != nil {
				logger.Error("failed to marshal nspro feedback", "error", err)
				return
			}
			if _, err := conn.Write(data); err != nil {
				logger.Error("failed to send nspro feedback", "error", err)
			}
		})
		defer clearOutputCallback()

		buf := make([]byte, InputWireSize)
		for {
			if _, err := io.ReadFull(conn, buf); err != nil {
				if err == io.EOF {
					logger.Info("client disconnected")
					return nil
				}
				return fmt.Errorf("read input state: %w", err)
			}

			var state InputState
			if err := state.UnmarshalBinary(buf); err != nil {
				return fmt.Errorf("unmarshal input state: %w", err)
			}
			ns.UpdateInputState(state)
		}
	}
}

func (h *handler) UpdateMetaState(meta string, dev *usb.Device) error {
	ns, ok := (*dev).(*NSPro)
	if !ok {
		return fmt.Errorf("expected nspro device")
	}

	metaState := *defaultMetaState()
	if err := json.Unmarshal([]byte(meta), &metaState); err != nil {
		return fmt.Errorf("unmarshal meta state: %w", err)
	}

	ns.SetMetaState(metaState)
	return nil
}

func (d *NSPro) metaSerialNumber() string {
	d.stateMu.Lock()
	defer d.stateMu.Unlock()
	if d.metaState == nil {
		return ""
	}
	return d.metaState.SerialNumber
}
